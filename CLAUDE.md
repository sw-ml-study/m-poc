...
`m-poc` is a mock-up of **Rosetta M**: a 3D stone whose faces are the
**Math**, the **proposed M code** and the **Visual** (structure + live data) of
one small ML computation, with training **time** as the fourth dimension.
[`docs/plan.md`](docs/plan.md) is the authority for scope;
[`docs/research.txt`](docs/research.txt) is the source discussion, of which
this repository implements only the visualization slice.

### What this repository is about

The first stone is one neuron trained by gradient descent
(`ŷ = wx + b`, `L = (y − ŷ)²`, `w ← w − η ∂L/∂w`). Rotate the stone and the
representation changes; scrub epochs and the numbers animate while the
equation, the M source and the dataflow stay put; click a token and its
counterparts highlight on every face.

### Rules that are enforced, not merely intended

- **M code is proposed, never run.** There is no M parser, lowering or
  evaluator here. The M face is a string rendered from the IR record. Where
  code has to execute, it is ordinary sw-MLPL run by the native `mlpl-repl`.
- **The three faces come from one record.** Math text, M text and the token
  table are rendered from the same IR in `lib/rosetta/`; never hand-write a
  face.
- **Numbers are real.** The epoch trace comes from sw-MLPL's `grad`, not
  from typed-in values, and the page replays `trace.json` rather than
  recomputing.
- **MLPL owns meaning; JavaScript owns pixels.** Geometry is emitted by MLPL
  as `positions/edges/colors/ids` arrays in the
  `sw-ml-study.native3d.line-scene` schema (the `../demo-extensions`
  approach); the page JS is a generic projector that knows nothing about
  neurons.
- **No demo vocabulary in `lib/`.** `lib/rosetta/` knows about equations,
  tokens, faces and scenes; `demos/one-neuron/` knows about neurons.
- **Glyphs are placeholders.** The proposed-M glyph set is provisional and
  every page and document that shows it says so.
- **No build toolchain for the page.** `pages/` is static HTML/CSS/JS plus
  copied fixtures; no Rust, WASM or bundler.
...
convention in the briefing above does not apply here unless asked for.
...
- the adjacent [`../sw-mlpl`](https://github.com/sw-ml-study/sw-mlpl) checkout
  ...
  override; measured against `mlpl-repl 0.22.0`. Its `svg(text, "equation")`
  renders the Math and M faces; `grad` produces the trace; `write_atomic`
  writes fixtures;
- `mlplunit` on `PATH`, an absolute `MLPLUNIT` override, or the adjacent
  `../../softwarewrighter/mlplunit/bin/mlplunit` checkout;
- `reg-rs` on `PATH`; baselines live in `tests/reg/` via
  `REG_RS_DATA_DIR`, so `reg-rs rebase` is a reviewed change;
- [`just`](https://github.com/casey/just);
- a browser for `just serve-site`. Nothing else.
