...
## This repository in one paragraph

`m-poc` is a mock-up of **Rosetta M**: a 3D stone whose faces are the
**Math**, the **proposed M code** and the **Visual** of one small ML
computation (one neuron trained by gradient descent), with training time as
the fourth dimension. M code is proposed and never run; everything that
executes is sw-MLPL run by the native `mlpl-repl`, and the page is static
HTML/CSS/JS replaying what MLPL emitted. [`docs/plan.md`](docs/plan.md) is
